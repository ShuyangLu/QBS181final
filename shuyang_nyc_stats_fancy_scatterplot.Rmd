...
ggplot(add_season_test,aes(x=`Day.Average.Temp`,y=`Case_Count_7Day_Avg`))+
  geom_point()+ #this controls the scatter plots
  labs(x="Temperature", y ="Number of COVID cases",title="Temperature ~ Number of COVID Scatter Plot")+
...
```{r}
## test log cases
ggplot(add_season_test,aes(x=`Day.Average.Temp`,y=`log_cases`,color=season,shape=season))+
  geom_point()+ #this controls the scatter plots
  labs(x="Humidity_index", y ="Number of COVID cases",title="Temperature ~ Number of COVID Scatter Plot")+theme_classic()+geom_smooth(aes(x=`Day.Average.Temp`,y=`log_cases`),method="lm",inherit.aes=F)
